...
enum DemoClientFactory {
  ...
  static func makeClient(mode: DemoCatalog.ClientMode) -> HTTPClient {
    ...
    case .mock:
      return .live(
        configuration: Self.makeHTTPConfiguration(mode: mode),
        transport: Self.makeMockTransport()
      )
  ...
  static func makeHTTPConfiguration(mode: DemoCatalog.ClientMode) -> ClientConfiguration {
    switch mode {
    case .mock:
  ...
  static func makeMockTransport() -> MockTransport {
    let routeState = DemoRouteState()
    return MockTransport { request async throws(NetworkError) -> RawResponse in
